...
// --- Helper Functions ---
const calculatePageChars = (page: ModulePage): number => {
    return page.content.reduce((acc, block) => {
        if (Array.isArray(block.content)) return acc + block.content.join('').length;
        if (typeof block.content === 'string') {
            if (block.type === 'image' || block.type === 'video') return acc;
...
const countPageImages = (page: ModulePage): number => page.content.filter(b => b.type === 'image').length;
...
const ModuleCreator: React.FC = () => {
    ...
    useEffect(() => {
        if (editingModule) {
            ...
            loadPages();
             ...
             setSelectedSeries(user?.series ? [user.series] : [SCHOOL_YEARS[0]]);
             setSelectedSubjects([]);
             setPages([{ id: Date.now(), title: 'Página 1', content: [] }]);
        }
    }, [editingModule, user, addToast]);
    ...
    const handleSave = async (isDraft: boolean) => {
        if (!title || selectedSeries.length === 0 || selectedSubjects.length === 0 || isSubmittingContent) {
        ...
        const moduleData: any = {
            ...
            pages,
            visibility: isDraft ? 'private' : 'public', // Simplify visibility for teachers
            classIds: [], 
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex justify-between items-center">
                 <div>
                     <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100 hc-text-primary">
                        {editingModule ? 'Editar Módulo' : 'Criar Módulo'}
                    </h2>
                     <p className="text-slate-500 dark:text-slate-400 mt-1">Crie conteúdo didático para suas turmas.</p>
                </div>
            ...
            <div className="flex justify-end space-x-4 pb-8">
                ...
                >
                     {isSubmittingContent ? <SpinnerIcon className="h-5 w-5 text-white" /> : <div className="h-5 w-5">{ICONS.plus}</div>}
